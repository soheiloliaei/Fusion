# Fusion v14.5 Master Prompt

## System Overview
Fusion v14.5 is a complete AI-Native Operating System with 31 specialized agents, synthetic reasoning, debug UI, agent choreographer, pattern testing, voice input, and web interface. The system now includes Sprint 5-9 features: debug visualization, multi-agent chains, pattern effectiveness testing, voice-to-text processing, and a complete Streamlit web application.
...
## Sprint 5-9 Features (v14.5)

### Debug UI System (`debug_ui.py`)
**Purpose**: Complete synthetic reasoning visualization during agent execution

**Features**:
- Real-time display of all 5 synthetic thoughts
- Internal questions visualization (5 per agent)
- Risk score monitoring (0.0-1.0)
- Fallback pattern activation tracking
- `--debug` CLI flag for verbose output
- Session analytics and debug logs

**Usage**:
```bash
python3 fusion.py --debug run vp_design "Design task"
```

### Agent Choreographer (`agent_choreographer.py`)
**Purpose**: Multi-agent chain execution with risk-aware stopping

**Features**:
- 5 predefined agent chains (design_pipeline, narrative_loop, strategy_chain, content_flow, product_journey)
- Sequential agent execution with context passing
- Automatic halt when risk_score > 0.9
- Execution analytics (duration, success rate, step completion)
- Chain results logging and analytics
...
### Pattern Testing System (`pattern_tester.py`)
**Purpose**: Live effectiveness testing of fallback patterns

**Features**:
- A/B comparison (normal vs pattern execution)
- Effectiveness scoring (0.0-1.0)
- Risk impact analysis
- Output change detection
- Pattern performance analytics
- Session result logging

**Usage**:
```bash
python3 fusion.py test-pattern evaluator fallback_metric_narrative "Test input"
```

**Available Patterns**:
- `fallback_clarify_then_critique`: Systematic clarification then step-by-step critique
- `fallback_metric_narrative`: Structured evaluation with accessibility, hierarchy, clarity metrics
- `fallback_soften_for_exec`: Executive-friendly tone and strategic framing
- `fallback_safe_design`: Conservative, minimal-risk design recommendations
- `fallback_user_centric`: User needs and accessibility focus
- `fallback_systematic`: Clear methodology with structured reasoning

### Voice Input System (`voice_input.py`)
**Purpose**: Voice-to-text agent processing with risk detection

**Features**:
- Multiple transcription methods (Whisper, Speech Recognition, Manual)
- Automatic method fallback
- Voice session logging
- High-risk confirmation (risk_score > 0.8)
- Complete voice-to-agent pipeline

**Usage**:
```bash
python3 fusion.py voice strategy_pilot --method auto --duration 10
```

**Methods**:
- `whisper`: OpenAI Whisper (best quality)
...
**Purpose**: Complete Streamlit interface for all Fusion functionality

**Features**:
- 5 complete pages: Run Agent, Agent Chains, Pattern Testing, Voice Input, Configuration
- Real-time debug visualization
- Agent selection (all 31 agents)
...
3. **🧩 Pattern Testing**: Live pattern effectiveness comparison
4. **🎤 Voice Input**: Voice-to-agent processing interface
5. **⚙️ Configuration**: Live config editing and analytics

### Synthetic Reasoning Layer (`synthetic_reasoner_agent.py`)
**Purpose**: Meta-agent providing introspection before main agent execution

**Features**:
- 5 synthetic thoughts per agent execution
- 5 internal questions for self-reflection
- Risk score calculation (0.0-1.0)
- Pattern trigger assessment
- LLM-powered reasoning (when configured)

**Integration**:
- Runs automatically before every agent
- Logs all synthetic meta information
- Triggers fallback patterns when risk_score > 0.65
- Provides complete introspection visibility

### Configuration System
**Files**:
- `agent_manifest.json`: All 31 agents documented with roles and fallback patterns
- `agent_chains.json`: Predefined agent chains configuration
- `fallback_trigger_config.json`: Risk routing and pattern mapping
- `pattern_registry.py`: Fallback pattern templates and metadata

### CLI Commands (v14.5)
```bash
# Debug mode
python3 fusion.py --debug run [agent] "[input]"

# Agent chains
python3 fusion.py chain [chain_name] "[input]"

# Pattern testing
python3 fusion.py test-pattern [agent] [pattern] "[input]"

# Voice input
python3 fusion.py voice [agent] --method [auto|whisper|manual] --duration [seconds]
...
- **Component Testing**: `test_web_app.py` validates all imports and functionality
- **Session Analytics**: Complete tracking of debug, chain, pattern, and voice sessions
- **Error Handling**: Robust fallbacks for missing dependencies
- **Modular Design**: Each Sprint 5-9 feature in separate files for maintainability

This master prompt provides comprehensive guidance for understanding and using Fusion v14.5, ensuring consistent behavior across all system components including the complete Sprint 5-9 AI-Native Operating System features.
